...
export default function Nav() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [{ data: profile }, access] = await Promise.all([
        supabase.from('profiles').select('name, role, location').eq('id', user.id).single(),
        fetchAccess(),
      ]);
      setMe({
  ...
  // Page visibility set by the owner (Admin -> Page access). Hidden until
  // loaded so a restricted role never sees links flash in and out.
  const show = (key: string) => (allowed ? allowed.has(key) : false);
  const showGroup = (keys: string[]) => keys.some((k) => show(k));
  ...
  return (
    ...
    >
      ...
      <nav style={{ flex: 1, overflowY: 'auto', padding: '14px 0' }}>
        ...
        {showGroup(['operations-dashboard', 'finance-dashboard', 'ceo-dashboard']) && (
          <div
            style={{
              fontFamily: "'IBM Plex Mono', monospace",
              fontSize: '9px',
        ...
        {showGroup(['notifications', 'vendor-verification', 'settings', 'admin']) && (
          <div
            style={{
              fontFamily: "'IBM Plex Mono', monospace",
              fontSize: '9px',
